# -*- coding: utf-8 -*-
from akad.ttypes import IdentityProvider, LoginResultType, LoginRequest, E2EEKeyChain, LoginType
from .server import Server
from .session import Session
from .callback import Callback
from .newqr import QRLogin
import rsa, os
...
class Auth(object):
    ...
    def loginWithQrCode(self):
        ...
        if APPTYPE == "ANDROIDLITE":
            result = newqr.loginWithQrCode("android_lite")
        elif APPTYPE == "IOSIPAD":
            result = newqr.loginWithQrCode("ios_ipad")
        elif APPTYPE == "CHROME":
            result = newqr.loginWithQrCode("chrome")
        elif APPTYPE == "DESKTOPWIN":
            result = newqr.loginWithQrCode("desktopwin")
        elif APPTYPE == "DESKTOPMAC":
            result = newqr.loginWithQrCode("desktopmac")
        else:
